...
#file.close()
model = fasttext.skipgram('full_text.txt.txt', 'model')
fasttext_vectors = {}
for i in range(len(node_info)):
    abstract = node_info[i][5].split()
    vector = np.zeros(100)
    for j in range(len(abstract)):
        vector += model[abstract[j]]
    fasttext_vectors[node_info[i][0]] = vector
...
#cosine distance of fasttext vectors
cosine_distance = []
...
for i in range(len(training_set_reduced)):
    ...
    cosine_distance.append(cosine_similarity(fasttext_vectors[source].reshape(1, -1), fasttext_vectors[target].reshape(1, -1))[0][0])
...
for i in range(len(testing_set)):
    ...
    cosine_distance_test.append(cosine_similarity(fasttext_vectors[source].reshape(1, -1), fasttext_vectors[target].reshape(1, -1))[0][0])
...
# convert list of lists into array
# documents as rows, unique words as columns (i.e., example as rows, features as columns)
testing_features = np.array([overlap_title_test,temp_diff_test,comm_auth_test, cosine_distance_test]).T
...
with open("fasttext_predictions.csv","w") as pred1:
